...
int accumulate_pop_stats(popstats *total, popstats *n) {
	...
	if (total->size == -1) {
		...
		total->best = (saved_ind **) MALLOC(total->bestn * sizeof(saved_ind *));
		...
		/* find the maximums. */
		if (n->maxnodes > total->maxnodes)
			total->maxnodes = n->maxnodes;
		if (n->maxdepth > total->maxdepth)
			total->maxdepth = n->maxdepth;
		if (n->maxhits > total->maxhits)
			total->maxhits = n->maxhits;

		/* find the minimums. */
		if (n->minnodes < total->minnodes)
			total->minnodes = n->minnodes;
		if (n->mindepth < total->mindepth)
			total->mindepth = n->mindepth;
		if (n->minhits < total->minhits)
			total->minhits = n->minhits;
		...
		temp = (saved_ind **) MALLOC(total->bestn * sizeof(saved_ind *));
...
void read_stats_checkpoint(multipop *mpop, ephem_const **eind, FILE *f) {
	...
	for (i = 0; i < mpop->size + 1; ++i) {
		...
		fscanf(f, "%d  %d %d %d %d %d  %d %d %d %d %d  %d %d %d %d %d\n",
				&(run_stats[i].size), &(run_stats[i].maxnodes),
				&(run_stats[i].minnodes), &(run_stats[i].totalnodes),
				&(run_stats[i].bestnodes), &(run_stats[i].worstnodes),
				&(run_stats[i].maxdepth), &(run_stats[i].mindepth),
				&(run_stats[i].totaldepth), &(run_stats[i].bestdepth),
				&(run_stats[i].worstdepth), &(run_stats[i].maxhits),
				&(run_stats[i].minhits), &(run_stats[i].totalhits),
				&(run_stats[i].besthits), &(run_stats[i].worsthits));
		...
		fscanf(f, "%d %d %d %d %d ", &(run_stats[i].bestgen),
				&(run_stats[i].worstgen), &(run_stats[i].bestpop),
				&(run_stats[i].worstpop), &(run_stats[i].bestn));
		run_stats[i].best = (saved_ind **) MALLOC(
				run_stats[i].bestn * sizeof(saved_ind *));
...
saved_ind ** write_saved_individuals(ephem_index *eind, FILE *f) {
	...
	index = (saved_ind **) MALLOC(saved_head->refcount * sizeof(saved_ind *));
...
void write_stats_checkpoint(multipop *mpop, ephem_index *eind, FILE *f) {
	...
	for (i = 0; i < mpop->size + 1; ++i) {
		...
		fprintf(f, "%d  %d %d %d %d %d  %d %d %d %d %d  %d %d %d %d %d\n",
				run_stats[i].size, run_stats[i].maxnodes, run_stats[i].minnodes,
				run_stats[i].totalnodes, run_stats[i].bestnodes,
				run_stats[i].worstnodes, run_stats[i].maxdepth,
				run_stats[i].mindepth, run_stats[i].totaldepth,
				run_stats[i].bestdepth, run_stats[i].worstdepth,
		...
		fprintf(f, " %f %f %f\n", run_stats[i].bestfit, run_stats[i].worstfit,
				run_stats[i].totalfit);
		/* write more integers. */
		fprintf(f, "%d %d %d %d %d ", run_stats[i].bestgen,
				run_stats[i].worstgen, run_stats[i].bestpop,
				run_stats[i].worstpop, run_stats[i].bestn);
